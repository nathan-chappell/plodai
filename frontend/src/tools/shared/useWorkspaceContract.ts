...
import type { ToolProviderWorkspaceSnapshot } from "../../lib/workspace-store";
...
function migrateLegacySnapshot(
  ...
  options: {
    ...
    allowedTabs: string[];
    toolNames?: string[];
    legacySnapshot: ToolProviderWorkspaceSnapshot | null;
  },
) {
  workspace.updateFilesystem((currentFilesystem) => {
    let nextFilesystem = ensureWorkspaceContractFilesystem(currentFilesystem, {
      toolProviderId: options.toolProviderId,
      toolProviderTitle: options.toolProviderTitle,
...
export function useWorkspaceContract(options: {
  ...
  const syncToolCatalog = useCallback((toolNames: string[]) => {
    options.workspace.updateFilesystem((filesystem) =>
      syncWorkspaceToolCatalog(filesystem, options.toolProviderId, toolNames),
    );
  }, [options.toolProviderId, options.workspace.updateFilesystem]);
