...
export async function buildReportAgentDemoScenario(): Promise<CapabilityDemoScenario> {
  ...
  return {
    ...
    initialPrompt: [
      ...
      "Start by listing the reports and make sure there is an active report.",
      ...
      "Have the specialist use dataset_id demo-report-sales to create one derived chartable artifact from that CSV using a simple grouped aggregate query: group by region, sum revenue as total_revenue, and sort the result by total_revenue descending.",
      "You must then delegate to the chart specialist and render one useful bar chart from that derived artifact.",
