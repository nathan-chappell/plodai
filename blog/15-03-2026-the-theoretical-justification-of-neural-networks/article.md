...
For this pass I simplified the experiment quite a bit.  I trained one small two-layer recurrent net with four hidden units per layer on strings of exact length twenty, evaluated it on lengths ten, twenty, and thirty, and fixed a tiny support of only one hundred twenty training strings from the start.  Forty belong to the initial random curriculum, forty pair valid strings with off-by-one counterexamples, and forty pair valid strings with balanced-but-invalid counterexamples.  The important thing is that I am not changing the formal language under consideration.  I am only changing which parts of that small support get weight as training proceeds.
...
*Story of the figure: the top panel makes the tiny-support curriculum easier to read.  The net improves as the support gets richer, but the bottom panel is the more interesting part: keep many individual probes fixed and watch their probabilities move while only the training distribution changes.  Color means problem class, dash means length, and the highlighted labels mark the probes that actually flip correctness at a phase boundary.  In this run the cleanest late recovery is `20O1`, a length-twenty off-by-one string that remains incorrectly accepted through phase 2 and then gets pushed decisively into the rejecting region once the balanced-invalid cohort is weighted in.  That is the whole point of the demo: a few counterexamples can reorganize the recognizer without changing the language itself.*

[Interactive training story chart](/blog-assets/theoretical-justification-of-neural-networks/rnn-training-story.html)

What I especially like here is that the balanced-invalid rollout does not merely add one more category of negative examples.  It changes the weighting on a few carefully chosen counterexamples, and that seems to repair part of the geometry of the recognizer.  In the run shown here, phase 3 finally corrects an off-by-one probe that the earlier training still treated as if simple local balance were enough.
...
*Story of the figure: rows show the end of phases 1, 2, and 3.  The first column is the same focus probe selected from the bifurcation chart, here the length-twenty off-by-one string `20O1`, and the neighboring columns are same-length valid and balanced-invalid companions.  The faint background cloud is the classifier's own top-layer state space, colored by its current estimate of `p(valid)`, while the overlaid traces show how the three strings move through that space.  The interpretation is only heuristic, but the picture is suggestive: one principal direction looks count-like, another looks more like the learned heuristic, and phase 3 finally reroutes `20O1` out of the accepting region.*
