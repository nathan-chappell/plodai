...
For this pass I simplified the experiment into a much cleaner two-beat story.  I trained one small two-layer recurrent net with eight hidden units per layer, optimized it with Adam at the default learning rate, evaluated it on lengths ten, twenty, and thirty, and used two training phases instead of four.  Phase 1 is a large random-only baseline: seven hundred sixty-eight training strings for ten epochs.  Phase 2 keeps that baseline and then adds just sixty-four examples from each counterexample family, running for twenty more epochs.  So the second phase is not a wholesale data replacement.  It is a relatively small, carefully chosen shock: off-by-one corruptions, balanced-but-invalid strings, and valid-prefix negatives that look locally fine but end unfinished.
...
*Story of the figure: the top panel now tells one simple story.  Phase 1 is a large random-only baseline, and phase 2 keeps that baseline while adding a tiny all-at-once counterexample shock.  The bottom panel is the more interesting part: instead of a tiny handpicked catalog, it tracks the full `2^10 = 1024` space of binary strings of length ten and watches their responses move while only the training distribution changes.  That lower vertical axis is shown in signed sigma units rather than raw probability, so small changes near the decision boundary become easier to see.  Green means a probe stays correct and valid throughout, yellow means it stays correct and invalid throughout, and red marks probes that are wrong at some point in training.  The dashed lines are the truly interesting ones: probes whose correctness turns more than once over the course of training, producing paths like `C -> I -> C` or `I -> C -> I`.  That is the whole point of the demo: a few counterexamples can reorganize the recognizer without changing the language itself.*

[Interactive training story chart](/blog-assets/theoretical-justification-of-neural-networks/rnn-training-story.html)

What I especially like here is that phase 2 does not merely add more negative examples in bulk.  It perturbs the training distribution with a very small counterexample set, and that seems to repair part of the geometry of the recognizer without erasing the baseline story.  In the stronger eight-unit run shown here, that second phase really does look like a bifurcation: some probes get repaired, some get destabilized, and the interesting ones trace out visible qualitative shifts.

![Focused hidden-state traces for a probe before and after the counterexample shock, together with nearby companion strings.](/blog-assets/theoretical-justification-of-neural-networks/rnn-transition-traces.png)

*Story of the figure: rows now show just the end of phases 1 and 2, so each 3D panel can be materially larger.  The first column is the focus probe selected from the bifurcation chart's exhaustive length-ten slice, and the neighboring columns are same-length valid and contrasting invalid companions.  The background mapper is the classifier's own top-layer state space in a 3D PCA view, rendered as small translucent points colored by signed sigma units rather than raw probability: negative sigma means reject-leaning, positive sigma means accept-leaning, and the midpoint is kept neutral instead of washed out.  The camera is fixed off-axis, the point radii are depth-scaled, and each panel now includes a much stronger axis frame so the static image reads clearly as a volume instead of a flat scatter.  Sparse numeric step labels mark a few anchor positions along each trace so it is easier to see how the strings move through that space.  The shared PCA note at the top tells you how much variance the three plotted directions capture.  The interpretation is only heuristic, but the picture is suggestive: one principal direction looks count-like, another looks more like the learned heuristic, and the interesting failures are not random noise.  They are structurally similar strings living close to a learned geometric boundary.*
