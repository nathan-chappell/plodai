...
import { buildChartData, buildChartOptions, getChartSurfaceStyle } from "../lib/chart";
...
export function DatasetChart({ spec, rows }: { spec: ClientChartSpec; rows: DataRow[] }) {
  ...
  return (
    <Wrapper $background={surface.background} $border={surface.border}>
      {hasRows ? (
        <>
          {spec.type === "line" ? <Line data={chartData as never} options={options as never} /> : null}
          {spec.type === "bar" ? <Bar data={chartData as never} options={options as never} /> : null}
          {spec.type === "pie" ? <Pie data={chartData as never} options={options as never} /> : null}
          {spec.type === "doughnut" ? <Doughnut data={chartData as never} options={options as never} /> : null}
          {spec.type === "scatter" ? <Scatter data={chartData as never} options={options as never} /> : null}
        </>
      ) : (
